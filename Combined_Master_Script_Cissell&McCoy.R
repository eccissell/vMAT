#Combined script for analyses presented in Cissell & McCoy in review: Top-heavy trophic structure within benthic viral dark matter
...
#Create a iGraph object from subset frma
...
#Modified for Phage-Only Paper (Original Code was for when they were combined and I didn't want to scrap it just in case)
###########
library(dplyr)
library(gdata)
library(tidyverse)
...
##############################################
########################
#Time to get the host and phage taxonomy and abundances going for some plots
#This is contingent on outputs from other R scripts in this manuscript
########################
##############################################
...
#Do an overall correlation now
clm1=lm(data=Phage_host_count_merge,Phage_host_count_merge$logP~Phage_host_count_merge$logH)
summary(clm1)
#Test assumptions
library("DHARMa")
check_overall_count_model <- simulateResiduals(fittedModel = clm1, n = 999)
plot(check_overall_count_model)
library(lmtest)
bptest(clm1)

#Normal, but fails homoskedasticity, look at shape of residuals
data.frame(y = rstandard(clm1),
           x = clm1$fitted.values) %>%
  ggplot(aes(x = x, y = y)) + 
  geom_point() +
...
wt=1/lm(sqrt(abs(clm1$residuals)) ~ clm1$fitted.values)$fitted.values^2

#perform weighted least squares regression
wls_clm=lm(Phage_host_count_merge$logP~Phage_host_count_merge$logH,
           data=Phage_host_count_merge,
           weights=(wt))
summary(wls_clm)

#Look at shape of residuals
data.frame(y = rstandard(wls_clm),
           x = wls_clm$fitted.values) %>%
  ggplot(aes(x = x, y = y)) + 
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dotted") +
  labs(title = "Standardized Residuals vs Fitted Values Plot")
min(abs(rstandard(wls_clm)))
max(abs(rstandard(wls_clm))) #3.940
4*min(abs(rstandard(wls_clm)))
#Test assumptions
library("DHARMa")
check_overall_count_model_weight <- simulateResiduals(fittedModel = wls_clm, n = 999)
plot(check_overall_count_model_weight)
library(lmtest)
bptest(wls_clm)

#Try random effect of mat
clmmatrnad=lmer(data=Phage_host_count_merge,Phage_host_count_merge$logP~Phage_host_count_merge$Time+logH+(1|Phage_host_count_merge$Mat))
summary(clmmatrnad)
#Test assumptions
library("DHARMa")
check_vmr_count_clmmatrnad <- simulateResiduals(fittedModel = clmmatrnad, n = 999)
plot(check_vmr_count_clmmatrnad)

Anova(clmmatrnad)#No significant temporal differences, but phyla differences still.
...
#Model logVMR across host abundnace
clm2=lm(data=Phage_host_count_merge,Phage_host_count_merge$logVMR~Phage_host_count_merge$logH)
summary(clm2)
#Test assumptions
library("DHARMa")
check_vmr_count_model <- simulateResiduals(fittedModel = clm2, n = 999)
plot(check_vmr_count_model)

#Try weighted regression
#define weights to use
wt2=1/lm(sqrt(abs(clm2$residuals)) ~ clm2$fitted.values)$fitted.values^2

#perform weighted least squares regression
wls_clm2=lm(Phage_host_count_merge$logVMR~Phage_host_count_merge$logH,
            data=Phage_host_count_merge,
            weights=(wt2))
summary(wls_clm2)
#Test assumptions
library("DHARMa")
check_vmr_count_model4 <- simulateResiduals(fittedModel = wls_clm2, n = 999)
plot(check_vmr_count_model4)
#SIMPSONS PARADOX IS STRONG IN THIS DATASET. I THINK THE 
vmr_H=ggplot(Phage_host_count_merge, aes(y=logVMR,x=logH))+
  geom_smooth(method=lm,se=F,
              aes(color=Phylum))+
  geom_point(alpha=0.8, aes(color=Phylum))+
  theme_classic()+
...
clm3=lmer(data=Phage_host_count_merge,Phage_host_count_merge$logVMR~Phage_host_count_merge$Phylum+(1|Phage_host_count_merge$Mat))
summary(clm3)
#Test assumptions
library("DHARMa")
check_vmr_count_model3 <- simulateResiduals(fittedModel = clm3, n = 999)
plot(check_vmr_count_model3)

Anova(clm3)
confint(clm3)

#Post-Hoc pairwise
time=pairwise.wilcox.test(Phage_host_count_merge$logVMR, Phage_host_count_merge$Time, p.adjust.method="bonferroni")
time
mat=pairwise.wilcox.test(Phage_host_count_merge$logVMR, Phage_host_count_merge$Mat, p.adjust.method="bonferroni")
mat
matlm=lm(Phage_host_count_merge$logVMR~Phage_host_count_merge$Mat)
summary(matlm)
#Test assumptions
library("DHARMa")
check_vmr_count_model3 <- simulateResiduals(fittedModel = matlm, n = 999)
plot(check_vmr_count_model3)
...
#Do an overall correlation now
clm1class=lm(data=Phage_host_count_merge_class,Phage_host_count_merge_class$logP~Phage_host_count_merge_class$logH)
summary(clm1class)
#Test assumptions
library("DHARMa")
check_overall_count_model_class <- simulateResiduals(fittedModel = clm1class, n = 999)
plot(check_overall_count_model_class)
library(lmtest)
bptest(clm1class)

#Normal, but fails homoskedasticity, look at shape of residuals
data.frame(y = rstandard(clm1class),
           x = clm1class$fitted.values) %>%
  ggplot(aes(x = x, y = y)) + 
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dotted") +
  labs(title = "Standardized Residuals vs Fitted Values Plot")
...
#Try weighted regression
#define weights to use
wt_class=1/lm(sqrt(abs(clm1class$residuals)) ~ clm1class$fitted.values)$fitted.values^2

#perform weighted least squares regression
wls_clm_class=lm(Phage_host_count_merge_class$logP~Phage_host_count_merge_class$logH,
                 data=Phage_host_count_merge_class,
                 weights=(wt_class))
summary(wls_clm_class)

#Test assumptions
library("DHARMa")
check_overall_count_model_weight_class <- simulateResiduals(fittedModel = wls_clm_class, n = 999)
plot(check_overall_count_model_weight_class)
library(lmtest)
bptest(wls_clm_class)
...
###########
#Diel Phage Activity Stats, including VMAR, NMDS of RNA space, and hive plots (or heatmaps honestly) of signigicant drivers of temporal clustering in NMDS space
###########
library(dplyr)
library(gdata)
library(tidyverse)
...
#######################
#We will be doing 3 types of NMDS
#First type, sum to viral family
...
#Do an overall correlation now
clm1mar=lm(data=Phage_host_count_mergemar,Phage_host_count_mergemar$logP~Phage_host_count_mergemar$logH)
summary(clm1mar)
#Test assumptions
library("DHARMa")
check_overall_count_modelmar <- simulateResiduals(fittedModel = clm1mar, n = 999)
plot(check_overall_count_modelmar)
library(lmtest)
bptest(clm1mar)
#Normal and homoskedastic #BP 0.5782
...
plot(check_vmr_count_model4)
#SIMPSONS PARADOX IS STRONG IN THIS DATASET. I THINK THE 
vmar_H=ggplot(Phage_host_count_mergemar, aes(y=logVMR,x=logH))+
  geom_smooth(method=lm,se=F,
              aes(color=Phylum))+
  geom_point(alpha=0.8, aes(color=Phylum))+
  theme_classic()+
...
#Model logVMAR across Phyla
clm3mar=lmer(data=Phage_host_count_mergemar,Phage_host_count_mergemar$logVMR~Phage_host_count_mergemar$Phylum+(1|Phage_host_count_mergemar$Mat))
summary(clm3mar)
#Test assumptions
library("DHARMa")
check_vmr_count_model3mar <- simulateResiduals(fittedModel = clm3mar, n = 999)
plot(check_vmr_count_model3mar)

Anova(clm3mar)#Significant differences by phyla in VMAR

#Model logVMAR across Time
clm4mar=lmer(data=Phage_host_count_mergemar,Phage_host_count_mergemar$logVMR~Phage_host_count_mergemar$Time+(Phage_host_count_mergemar$Time|Phage_host_count_mergemar$Phylum))
summary(clm4mar)
#Test assumptions
library("DHARMa")
check_vmr_count_model4mar <- simulateResiduals(fittedModel = clm4mar, n = 999)
plot(check_vmr_count_model4mar)

Anova(clm4mar)#No significant temporal differnces, but random effect variance large, try multiple predictors

#Model logVMAR across Time and phyla
clm5mar=lmer(data=Phage_host_count_mergemar,Phage_host_count_mergemar$logVMR~Phage_host_count_mergemar$Time+Phage_host_count_mergemar$Phylum+(Phage_host_count_mergemar$Phylum|Phage_host_count_mergemar$Mat))
summary(clm5mar)
#Test assumptions
library("DHARMa")
check_vmr_count_model5mar <- simulateResiduals(fittedModel = clm5mar, n = 999)
plot(check_vmr_count_model5mar)

Anova(clm5mar)#No significant temporal differences, but phyla differences still.

#Subset to just cyano and look for temporal pattern
vmar_cyanosub=Phage_host_count_mergemar%>%
  subset(Phylum=="Cyanobacteria")
clm6mar=lmer(data=vmar_cyanosub,vmar_cyanosub$logVMR~vmar_cyanosub$Time+(1|vmar_cyanosub$Mat))
summary(clm6mar)
#Test assumptions
library("DHARMa")
check_vmr_count_model6mar <- simulateResiduals(fittedModel = clm6mar, n = 999)
plot(check_vmr_count_model6mar)
Anova(clm6mar) #Significant differences across times, visualize with boxes
timecyano=pairwise.wilcox.test(vmar_cyanosub$logVMR, vmar_cyanosub$Time, p.adjust.method="bonferroni")
timecyano
#Plot cyano time subset
ggplot(vmar_cyanosub, aes(y=logVMR,x=Time))+
  geom_boxplot(aes(fill=Time),
               outlier.shape = NA,
               size=1,
               alpha=0.7)+
  ...
  #  geom_point(aes(color=Time),position=position_jitterdodge(0.2),pch=19,
  #             alpha=0.9,
  #             size=1.5)+
  theme_classic()+
  theme(axis.text = element_text(size=12,color="black"),
...
#scale_fill_viridis_d(direction=-1)+
#scale_color_viridis_d(direction=-1)
dev.print(png,fil="vmr_time_cyanobox.png",type="quartz",antialias="default",width=6.5,
          height=6.5,units="in",res=1300)

#Post-Hoc pairwise
#Create subset dataframes for these tests
x=pairwise.wilcox.test(Phage_host_merge_subset$logVMR, Phage_host_merge_subset$Phylum, p.adjust.method="bonferroni")
y=as.data.frame(x$p.value)
y=format(y,scientific=F)
...
#plot logVMR across Phyla overall with pairwise significance
vmr_boxmar=ggplot(Phage_host_count_mergemar, aes(y=logVMR,x=fct_rev(Phylum)))+
  geom_hline(yintercept=0,
             linetype="dashed",
             col="black",
             alpha=0.6)+
  ...
  coord_flip()
vmr_boxmar
ggsave("/Users/cissell/Desktop/vmr_boxmar.svg",width=6.5,height=4.5,units="in",vmr_boxmar)

#Regress VMAR against VMR
#Make united dataframe
Phage_host_count_mergemar_mod1=Phage_host_count_mergemar%>%
  mutate(logVMAR=logVMR)%>%
  subset(!Time=="2")%>%
  subset(!Time=="4")%>%
  select(c(Phylum,Mat,Time,logVMAR))
Phage_host_count_merge_mod1=Phage_host_count_merge%>%
  select(c(Phylum,Mat,Time,logVMR))
vmar_vmr=merge(Phage_host_count_mergemar_mod1,
               Phage_host_count_merge_mod1,by=c("Phylum","Mat","Time"))

#Model logVMAR against logVMR
clmll=lm(data=vmar_vmr,vmar_vmr$logVMAR~vmar_vmr$logVMR)
summary(clmll)
#Test assumptions
library("DHARMa")
check_vmr_count_model <- simulateResiduals(fittedModel = clmll, n = 999)
plot(check_vmr_count_model)
bptest(clmll)
#Fails homoskedastic
#Try weighted regression
#define weights to use
wtmar=1/lm(sqrt(abs(clmll$residuals)) ~ clmll$fitted.values)$fitted.values^2

#perform weighted least squares regression
wls_clmll=lm(vmar_vmr$logVMAR~vmar_vmr$logVMR,
             data=vmar_vmr,
             weights=(wtmar))
summary(wls_clmll)
#Test assumptions
library("DHARMa")
check_vmr_count_model <- simulateResiduals(fittedModel = wls_clmll, n = 999)
plot(check_vmr_count_model)
bptest(wls_clmll)

#Model logVMAR against logVMR with mat as random
clmll2=lmer(data=vmar_vmr,vmar_vmr$logVMAR~vmar_vmr$logVMR+(vmar_vmr$logVMR|vmar_vmr$Mat))
summary(clmll2)
#Test assumptions
library("DHARMa")
check_vmr_count_model <- simulateResiduals(fittedModel = clmll2, n = 999)
plot(check_vmr_count_model)
bptest(clmll2)
#Fails homoskedastic
fixef(clmll2,condVars=T)
ranef(clmll2,condVar=T)
coef(clmll2)
confint(clmll2)

overall_p_hmar=ggplot(vmar_vmr, aes(y=logVMAR,x=logVMR))+
  geom_smooth(method=lm,se=T,color="black",
              aes(weight=wtmar))+
  geom_point(alpha=0.8, aes(color=Phylum))+
  theme_classic()+
  ...
  theme(legend.position = "none")
overall_p_hmar
ggsave("/Users/cissell/Desktop/vmar~vmr.svg",width=3.25,height=3.25,units="in",overall_p_hmar)
...
#Do an overall correlation now
clm1mar_class=lm(data=Phage_host_count_mergemar_class,Phage_host_count_mergemar_class$logP~Phage_host_count_mergemar_class$logH)
summary(clm1mar_class)
#Test assumptions
library("DHARMa")
check_overall_count_modelmar_class <- simulateResiduals(fittedModel = clm1mar_class, n = 999)
plot(check_overall_count_modelmar_class)
library(lmtest)
bptest(clm1mar_class)
#Normal and homoskedastic #BP p= 0.6804
...
clm3mar_class=lmer(data=Phage_host_count_mergemar_class,Phage_host_count_mergemar_class$logVMR~Phage_host_count_mergemar_class$Class+(1|Phage_host_count_mergemar_class$Mat))
...
Anova(clm3mar_class)#Significant differences by phyla in VMAR
...
vmr_boxmar_class=ggplot(Phage_host_count_mergemar_class, aes(y=logVMR,x=fct_rev(Class)))+
  ...
  coord_flip()
vmr_boxmar_class
ggsave("/Users/cissell/Desktop/vmr_boxmar_class.svg",width=6.5,height=4.5,units="in",vmr_boxmar_class)
